...
async def syntax_check(email):
    ...
    if "@" in email:
        if re.match(EMAIL_REGEX, email):
            ...
            data.append(response.create_response(email=email,status=constant.Status.UNDELIVERABLE.value,reason=constant.Reason.INVALID_EMAIL.value))
    else:
        data.append({
            "email": f"{email}",
        ...
        if status >= 200 and status <= 250:
            ...
            data.append(
                response.create_response(
                    email=email,
                    status=constant.Status.UNDELIVERABLE.value
                    ,reason=constant.Reason.INVALID_EMAIL.value
                ))
        #     data.append({
        #     "email":email,
